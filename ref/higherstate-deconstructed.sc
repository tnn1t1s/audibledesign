...
// Play the pattern with random selection and bursts
Pbind(
    \instrument, \tb303,
    \freq, Pshuf(~patternFreq.collect({ |note| note[0] }), 4), // Randomly shuffle notes
    \dur, Prand([0.05, 0.1, 0.15, 0.2], inf), // Randomly select short durations
    \cutoff, 1000,
    \res, 0.95,
    \envMod, 0.5,
    \accent, Pshuf(~patternFreq.collect({ |note| note[3] }), 4), // Randomly shuffle accents
	\slide, Pshuf(~patternFreq.collect({ |note| note[4] }), 4), // Randomly shuffle slides
    \legato, Pshuf(~patternFreq.collect({ |note| if (note[4] == 1) { 1.1 } { 0.9 } }), 4) // Randomly shuffle legato values
).play(~clock);
